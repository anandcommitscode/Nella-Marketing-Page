...
export default function WaitlistView({ onNavigate }: WaitlistViewProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setLoading(true);
    // Simulate real API signup delay
    setTimeout(() => {
      setLoading(false);
      setHasJoined(true);
      // Save locally to simulate state persistence
      localStorage.setItem('nella_waitlist_email', email);
    }, 850);
  };

  return (
    <div className="lowercase text-left max-w-xl mx-auto px-6 py-16 space-y-12">
      {/* Opener */}
      ...
      <div className="text-center">
        <button
          onClick={() => onNavigate('privacy-principles')}
